...
class Social extends Extension {
  ...
  toHtml(data) {
    const { items, width, height, textAlign, containerPadding } = data;
    return `<div>
      <div style="padding:${containerPadding};text-align:${textAlign}">
        ${items.map(item => `<a key='${item.guid}' href='${item.url}'><img style='height:${height}px;width:${width}px;' src='${item.icon}'></img></a>`).join('')}
      </div>
    </div>`;
  }
  ...
  getProperties(values, update) {
    const { items, width, height, textAlign, containerPadding } = values;
  ...
  render() {
    ...
    return <div className="ds_content_social">
      <div className="ds_content_social_container" style={{
        ...
        {items.map(item => <a key={item.guid} href={item.url}><img style={{
          width,
          height,
        }} src={item.icon}></img></a>)}
      </div>
